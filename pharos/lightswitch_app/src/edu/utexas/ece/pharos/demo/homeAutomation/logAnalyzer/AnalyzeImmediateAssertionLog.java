 ...
 * immediate cyber-physical assertions.
 * 
 * @author Chien-Liang Fok
 */
public class AnalyzeImmediateAssertionLog {
	 ...
	 */
	public AnalyzeImmediateAssertionLog() {

		// Read the log files...
		FilenameFilter filter = new FilenameFilter() {
		    public boolean accept(File dir, String name) {
		        return name.startsWith("LightSwitchAppImmediate") && name.contains(".log");
		    }
		};
...
//		}
	...
	private void computeAverageDelta() {
		...
		System.out.println("Average actual delta: " + df.format(avg) + " +- " + df.format(conf95) 
				+ " (sample size = " + df.format(deltas.size()) + ")");
	}
	...
	private Round readFile(String fileName) {
		...
		// Whether the program is waiting for a sensor reading.
//		boolean waitingForSensor = false;
		...
		try {
			while ((line = br.readLine()) != null){
				...
				if (line.contains("Received AmbientLight message")) {
					...
					tokens = line.split("\\s");
//					long secs = Integer.valueOf(tokens[4]);
					
					line = br.readLine();
					tokens = line.split("\\s");
//					long nsecs = Integer.valueOf(tokens[4]);
...
//						currAssertion.sensorTime = (long)((((long) secs) * 1000000000 + nsecs) / 1e6);
//					}
				}
...
//				else if (line.contains("Waiting for new ambient light measurement")) {
//					tokens = line.split("[\\[\\]]");
//					long timestamp = Long.valueOf(tokens[1]);
//					currAssertion.sensorWaitTime = timestamp;
...
//				}
	...
	public static void main(String[] args) {
		new AnalyzeImmediateAssertionLog();
	}

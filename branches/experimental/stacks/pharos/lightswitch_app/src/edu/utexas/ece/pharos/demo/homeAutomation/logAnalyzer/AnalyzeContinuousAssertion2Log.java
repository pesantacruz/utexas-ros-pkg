 ...
 * continuous cyber-physical assertions.
 * 
 * @author Chien-Liang Fok
 */
public class AnalyzeContinuousAssertion2Log {
	 ...
	 */
	public AnalyzeContinuousAssertion2Log() {

		// Read the log files...
		FilenameFilter filter = new FilenameFilter() {
		    public boolean accept(File dir, String name) {
		        return name.startsWith("LightSwitchAppContinuous2") && name.contains(".log")
		        	&& !name.contains("Fail");
		    }
		};
...
//		computeAssertionLatency();
		computeAssertionExecCount();
	}

	private void computeAssertionExecCount() {
		Vector<Double> execCounts = new Vector<Double>();
		
		for (int i=0; i < rounds.size(); i++) {
			Round currRound = rounds.get(i);
			execCounts.add(((double)currRound.assertion.getNumEvaluations() / (double)currRound.assertion.getDuration()) * 1000.0);
		}
		
		double avg = Stats.getAvg(execCounts);
		double conf95 = Stats.getConf95(execCounts);
		
		DecimalFormat df = new DecimalFormat("#.##");
		System.out.println("Average assertion executions per second: " + df.format(avg) + " +- " + df.format(conf95)
				+ " (sample size = " + execCounts.size() + ")");
	}
...
//	}
	...
	private void analyzeTotalExecTime() {
		...
		while (e.hasMoreElements()) {
			...
			measurements.add((double)(currRound.assertion.endTime - currRound.assertion.startTime));
		}
	...
	private Round readFile(String fileName) {
		...
		// Whether the light was turned on and we should do one final evaluation of the continuous assertion.
		boolean doFinalEvaluation = false;
		...
		try {
			while ((line = br.readLine()) != null){
				...
				if (line.contains("Received AmbientLight message")) {
					...
					tokens = line.split("[\\[\\]]");
					long timestamp = Long.valueOf(tokens[1]);
					result.assertion.startTime = timestamp;
				}
...
//				}
					...
					if (result.assertion.endTime == Long.MIN_VALUE) {
						tokens = line.split("[\\[\\]\\s]");
						long timestamp = Long.valueOf(tokens[3]);
						result.assertion.evaluationTimes.add(timestamp);
						if (doFinalEvaluation)
							result.assertion.endTime = timestamp;
					}
				} 
//				else if (line.contains("Aborting the assertion")) {
//					tokens = line.split("[\\[\\]\\s]");
//					currAssertion.endTime = Long.valueOf(tokens[1]);
//					currAssertion.numEvaluations = Integer.valueOf(tokens[9]);
...
//				}
				...
				else if (line.contains("The assertion was evaluated")) {
	...
	class Round {
		...
		// there is only one continuous assertion per round
...
//		}
	...
	class Assertion {
		...
		Vector<Long> evaluationTimes = new Vector<Long>();
		
		public Assertion() {
		}
		
		public int getNumEvaluations() {
			return evaluationTimes.size();
		}
		
		public long getDuration() {
			return evaluationTimes.get(evaluationTimes.size()-1) - startTime;
		}
		
		public String toString() {
			return "startTime = " + startTime + ", numEvaluations = " + getNumEvaluations() 
				+ ", duration = " + getDuration() + ", frequency = " + (getNumEvaluations() / getDuration() * 1000.0);
		}
	}
	...
	public static void main(String[] args) {
		new AnalyzeContinuousAssertion2Log();
	}
}
